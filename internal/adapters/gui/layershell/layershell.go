...
// SetKeyboardExclusive re-asserts exclusive keyboard mode on a layer-shell
// window. Useful when a previous layer-shell surface's cleanup races with
// the new one.
func SetKeyboardExclusive(window *gtk.Window) {
	if !layershell.Available() || !layershell.IsSupported() {
		return
	}
